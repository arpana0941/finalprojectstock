...
# Step 4: Locate the second <tbody> which contains quarterly revenue table
table_body = soup.find_all("tbody")[1]

# Step 5 & 6: Loop through rows and extract data
...
# Step 4: Locate the second <tbody> — where quarterly GME revenue is stored
table_body = soup.find_all("tbody")[1]

# Step 5: Extract rows and populate DataFrame
for row in table_body.find_all("tr"):
    cols = row.find_all("td")
    if len(cols) == 2:
        date = cols[0].text.strip()
...
# Optional: Remove empty strings and nulls if any
gme_revenue.dropna(inplace=True)
gme_revenue = gme_revenue[gme_revenue["Revenue"] != ""]

# Step 7: Display to verify
...
def make_graph(stock_data, revenue_data, stock):
    ...
    stock_data = stock_data[stock_data.Date <= "2021-04-30"]
    ...
    fig.update_layout(height=600, showlegend=False, title_text=f"{stock} Stock and Revenue")
    fig.show()
